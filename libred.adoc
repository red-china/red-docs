...
Red 值可以通过 libRed 函数调用被返回，以 32 位的 *不透明（opaque）* 引用表示。这些引用的生命周期很短，所以它们只适合于在受限的局部范围内使用，比如传递那个引用到另一个 libRed 的函数调用。可以把这种引用设置给宿主变量，它应 *紧接其后* 被使用。这些引用使用一个特定的内存管理器，它将仅在接下来的大约 50 次 API 调用内保持引用有效。例：
...
定义一个新的 Red 例程叫做 _name_，以 _spec_ 作为规格区块，_func-ptr_ C 函数指针做为主体。C 函数 *必须* 返回一个 Red 值（`redUnset()` 可以用来表示没有使用返回值）。
...
将 _value_ 追加到 _series_ 中，并返回该序列头部。
...
修改 _series_ 中的 _value_，并返回修改的部分之后的序列。
...
返回 _series_ 相对于头部的当前的索引，或返回语境中的单词。
