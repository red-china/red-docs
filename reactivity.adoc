...
静态关系的另一种形式可以用 `is` 操作符定义，它把该响应求得的值赋给一个（在任意语境中的）单词。
...
一个响应由 `/unlink` 修饰词移除，以下之一作为一个 `<source>` 参数：

* `'all` 单词，会移除所有该响应创建的响应关系。
* 一个对象值，仅会移除以该对象为响应源的关系。
* 一个对象列表，仅会移除以这些对象为响应源的关系。
...
`is` 创建一个响应公式，其结果将被赋给一个单词。`<code>` 代码区块可以包含外部响应器的字段，如果在响应器的响应体区块中使用，还可以包含对包装对象的字段的引用。
...
从对象体区块构造一个新的响应对象。将返回的对象中的一个字段设为新值会触发为该字段定义的响应。

NOTE: 该对象体可以包含 `is` 表达式。
...
从对象体区块构造一个新的响应对象。将返回的对象中的一个字段设为新值或修改该字段指向的序列（包括嵌套的序列）会触发为该字段定义的响应。
