...
NOTE: `miter-bevel` 模式将根据斜角长度自动选择其中一个接合模式（详情参考https://msdn.microsoft.com/en-us/library/windows/desktop/ms534148%28v=vs.85%29.aspx[此页面]）。
...
<start>    : 裁剪区域（area）的左上角点 (pair!)
<end>      : 裁剪区域（area）的右下角点 (pair!)
<mode>     : （可选）裁剪区域（regions）之间的合并模式 (word!)
<commands> : （可选）Draw 方言命令.
<shape>    : Shape 方言命令.
----
...
在 Draw 代码内部，可以使用区块任意地对命令进行分组，语义保持不变。目前这仅是个语法糖，它允许对命令进行更轻松的分组操纵（尤其是提取/插入/删除）。可以使用空区块。

== 源位置（Source position）

在 Draw 代码中，设词可以放在命令之间，用来记录在 Draw 区块中当前的位置，并能够在以后轻松地访问。

NOTE: 如果设词之前的绘制区块长度被改变了，原有的位置会随之改变，所以设词不再会指向相同的参照物。

== Draw 函数（Draw function）

可以使用 `draw` 函数将 Draw 块直接渲染到图像中。
